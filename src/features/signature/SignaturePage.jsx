...
export default function SignaturePage({ token }) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <main className="mx-auto grid max-w-6xl gap-4 px-3 py-4 sm:gap-6 sm:px-6 sm:py-6 lg:grid-cols-[minmax(0,1fr)_380px]">
        <section className="min-w-0 overflow-hidden rounded-xl bg-white shadow-xl ring-1 ring-slate-200 sm:rounded-2xl"><div className="border-b border-slate-200 px-4 py-3 sm:px-5 sm:py-4"><p className="text-sm font-black">{document.documentType === 'consulting' ? 'Proposta de Assessoria Especializada' : 'Orçamento de Assistência Técnica'}</p><p className="mt-1 text-xs leading-relaxed text-slate-500">Leia o documento inteiro antes de assinar. Válido até {formatExpiration(document.expiresAt)}.</p></div><div className="h-[48vh] min-h-[360px] max-h-[620px] bg-slate-200 sm:h-[70vh] sm:min-h-[580px]"><PDFViewer width="100%" height="100%" showToolbar={false}>{previewDocument}</PDFViewer></div></section>
        <aside className="h-fit min-w-0 rounded-xl bg-white p-4 shadow-xl ring-1 ring-slate-200 sm:rounded-2xl sm:p-5"><div className="flex items-center gap-2"><FileSignature className="shrink-0 text-blue-700" size={21} /><h2 className="text-lg font-black">Assinar documento</h2></div><p className="mt-2 text-sm leading-relaxed text-slate-600">Preencha seus dados e desenhe sua assinatura para registrar o aceite eletrônico.</p><form onSubmit={submit} className="mt-5 grid gap-4"><label className="block"><span className="text-xs font-black uppercase tracking-wide text-slate-600">Nome completo *</span><input value={signer.name} onChange={(event) => setSigner((current) => ({ ...current, name: event.target.value }))} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" autoComplete="name" required /></label><label className="block"><span className="text-xs font-black uppercase tracking-wide text-slate-600">CPF ou CNPJ *</span><input value={signer.document} onChange={(event) => setSigner((current) => ({ ...current, document: event.target.value }))} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" inputMode="numeric" required /></label><div><span className="text-xs font-black uppercase tracking-wide text-slate-600">Sua assinatura *</span><div className="mt-1"><SignaturePad onChange={setSignatureDataUrl} /></div></div><label className="flex items-start gap-2 text-xs leading-relaxed text-slate-600"><input type="checkbox" checked={signer.accepted} onChange={(event) => setSigner((current) => ({ ...current, accepted: event.target.checked }))} className="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-300 text-blue-700" required /><span>Declaro que li o documento, conferi os dados acima e aceito eletronicamente seu conteúdo.</span></label>{submitState.error ? <div className="rounded-lg bg-red-50 p-3 text-sm font-bold text-red-700">{submitState.error}</div> : null}<button type="submit" disabled={!canSubmit} className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 py-3 text-sm font-black text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-600">{submitState.loading ? <><LoaderCircle className="animate-spin" size={17} /> Registrando…</> : <><FileSignature size={17} /> Assinar documento</>}</button><p className="text-center text-[11px] leading-relaxed text-slate-500">Esta é uma assinatura eletrônica simples. O sistema registrará o aceite, a data e as informações técnicas da assinatura.</p></form></aside>
      </main>
    </div>
  );
}
